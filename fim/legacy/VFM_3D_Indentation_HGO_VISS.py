    ...
    for i in range(rows):
        for j in range(cols):
           for k in range(deps):
                ...
                sigma_aniso=(2*k1*E_iso/J**(5/3))*(np.exp(k2* E_iso**2 )) *(kappa*b + (1-3*kappa)*A4 + (-1/3)*np.eye(3)*(kappa*I1 + (1-3*kappa)*I4))
...
def main():
    ...
    res_1 = least_squares(residual_3,initial_guess,bounds=bnds,args=(tensor_displacement_list, X,Y,Z,k1,k2,volume_matrix,Force))
    print(res_1.x)
