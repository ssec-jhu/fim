...
def U_star_x_sin_devX(x,y,z):
    ...
    if d==0:
        ...
        return c*t* ((np.sin(2*np.pi * d / (L/2))) / d - x**2 * (np.sin(2*np.pi * d / (L/2))) / d**3 + x**2 * (np.cos(2*np.pi * d / (L/2))) * 2*np.pi / (d**2 * L/2))
    else:
        return 0
...
def U_star_x_sin_devY(x,y,z):
    ...
    if d==0:
        ...
        return c*x*t * ((-y/d**3)*(np.sin(2*np.pi * d / (L/2))) + (np.cos(2*np.pi * d / (L/2)))*2*np.pi*y/(d**2 * (L/2)))
    else:
        return 0
...
def U_star_y_sin_devX(x,y,z):
    ...
    if d==0:
        ...
        return c*y*t * ((-x/d**3)*(np.sin(2*np.pi * d / (L/2))) + (np.cos(2*np.pi * d / (L/2)))*2*np.pi*x/(d**2 * (L/2)))
    else:
        return 0
...
def U_star_y_sin_devY(x,y,z):
    ...
    if d==0:
        ...
        return c*t* ((np.sin(2*np.pi * d / (L/2))) / d - y**2 * (np.sin(2*np.pi * d / (L/2))) / d**3 + y**2 * (np.cos(2*np.pi * d / (L/2))) * 2*np.pi / (d**2 * L/2))
    else:
        return 0
...
def U_star_z_cos_devZ(x,y,z):
    ...
    if d<=a:
        ...
        return c * (np.cos(np.pi/2 *(d-a)/(L/2-a)))/H + c* t * (-(np.sin(np.pi/2 *(d-a)/(L/2-a)))) * np.pi/2 *  (d-L/2)/(L/2-a)**2 * a_0/H
    else: 
        return 0
...
def read_output_file(file_path):
    ...
    df=df.drop(columns=['ODB Name', 'Step', 'Frame', 'Section Name', 'Material Name', 'Section Point','X','Y','Z'],axis=1)
        ...
        for j in range(cols):
           for k in range(deps):
            ...
            # Calculate X1 and X2 for each centroid
                ...
                du_star_1_dX=np.array([[du1_dX1_1,du1_dX2_1,du1_dX3_1],[du2_dX1_1,du2_dX2_1,du2_dX3_1],[du3_dX1_1,du3_dX2_1,du3_dX3_1]])
                du_star_2_dX=np.array([[du1_dX1_2,du1_dX2_2,du1_dX3_2],[du2_dX1_2,du2_dX2_2,du2_dX3_2],[du3_dX1_2,du3_dX2_2,du3_dX3_2]])
                du_star_3_dX=np.array([[du1_dX1_3,du1_dX2_3,du1_dX3_3],[du2_dX1_3,du2_dX2_3,du2_dX3_3],[du3_dX1_3,du3_dX2_3,du3_dX3_3]])
                du_star_4_dX=np.array([[du1_dX1_4,du1_dX2_4,du1_dX3_4],[du2_dX1_4,du2_dX2_4,du2_dX3_4],[du3_dX1_4,du3_dX2_4,du3_dX3_4]])
                du_star_5_dX=np.array([[du1_dX1_5,du1_dX2_5,du1_dX3_5],[du2_dX1_5,du2_dX2_5,du2_dX3_5],[du3_dX1_5,du3_dX2_5,du3_dX3_5]])
...
def senstivity_full(tensor_displacement_list, matrix,E1,E2,v12,v23,Gt,undeformed_centroids,cube_size,Force,deviation):
    ...
    phi_base = calculate_VWS(tensor_displacement_list, matrix, E1, E2, v12, v23, Gt, undeformed_centroids, cube_size,Force)
  
    phi_E1_1 = calculate_VWS(tensor_displacement_list, matrix, E1_1, E2, v12, v23, Gt, undeformed_centroids, cube_size,Force)
    phi_E2_1 = calculate_VWS(tensor_displacement_list, matrix, E1, E2_1, v12, v23, Gt, undeformed_centroids, cube_size,Force)
    phi_v12_1 = calculate_VWS(tensor_displacement_list, matrix, E1, E2, v12_1, v23, Gt, undeformed_centroids, cube_size,Force)
    phi_v23_1 = calculate_VWS(tensor_displacement_list, matrix, E1, E2, v12, v23_1, Gt, undeformed_centroids, cube_size,Force)
    phi_Gt_1 = calculate_VWS(tensor_displacement_list, matrix, E1, E2, v12, v23, Gt_1_dev, undeformed_centroids, cube_size,Force)
...
def calculate_phi(args):
    ...
    alpha=0.005
    return res_regularised(tensor_displacement_list, matrix, E1, E2, v12, v23, Gt, undeformed_centroids, cube_size, Force,alpha)

def res_regularised(tensor_displacement_list, matrix,E1,E2,v12,v23,Gt,undeformed_centroids,cube_size,Force,alpha):
    ...
    phi_E1_1=np.abs(calculate_VWS(tensor_displacement_list, matrix,E1_1,E2,v12,v23,Gt,undeformed_centroids,cube_size,Force))
    phi_E1_2=np.abs(calculate_VWS(tensor_displacement_list, matrix,E1_2,E2,v12,v23,Gt,undeformed_centroids,cube_size,Force))
    phi_E2_1=np.abs(calculate_VWS(tensor_displacement_list, matrix,E1,E2_1,v12,v23,Gt,undeformed_centroids,cube_size,Force))
    phi_E2_2=np.abs(calculate_VWS(tensor_displacement_list, matrix,E1,E2_2,v12,v23,Gt,undeformed_centroids,cube_size,Force))
    # print(phi_E1_1,phi,phi_E1_2)
    # print(((phi_E1_1 + phi_E1_2 -2*phi)/dev),((phi_E2_1 + phi_E2_2 - 2*phi)/dev))
    # print(phi_E2_1,phi_E2_2)
    
    phi_reg=phi + alpha*(np.abs((phi_E1_1 + phi_E1_2 -2*phi)/dev)+ np.abs((phi_E2_1 + phi_E2_2 - 2*phi)/dev))
...
def main():
    ...
    dUx_dx, dUy_dx, dUz_dx, dUx_dy, dUy_dy, dUz_dy, dUx_dz, dUy_dz, dUz_dz = central_differentiation(Ux_enlarged, Uy_enlarged, Uz_enlarged,X_enlarged,Y_enlarged,Z_enlarged)
    tensor_displacement_list = map_elements_to_centraldiff(dUx_dx, dUy_dx, dUz_dx, dUx_dy, dUy_dy, dUz_dy, dUx_dz, dUy_dz, dUz_dz)
    ...
    res_1 = least_squares(residual,initial_guess,bounds=bnds,args=(tensor_displacement_list, X,Y,Z,volume_matrix,Force))
    print(res_1.x)
